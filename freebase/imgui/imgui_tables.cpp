...
// [SECTION] Tables: options (.ini data)
...
//    | TableResetoptions()                    - on options reset
//    | TableLoadoptions()                     - on options load
...
inline ImGuiTableFlags TableFixFlags(ImGuiTableFlags flags, ImGuiWindow* outer_window)
{
    ...
    // Adjust flags: disable saved options if there's nothing to save
    if ((flags & (ImGuiTableFlags_Resizable | ImGuiTableFlags_Hideable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Sortable)) == 0)
        flags |= ImGuiTableFlags_NoSavedoptions;

    // Inherit _NoSavedoptions from top-level window (child windows always have _NoSavedoptions set)
    if (outer_window->RootWindow->Flags & ImGuiWindowFlags_NoSavedoptions)
        flags |= ImGuiTableFlags_NoSavedoptions;
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    if (table->RawData == NULL)
    {
        ...
        table->IsInitializing = table->IsoptionsRequestLoad = true;
    }
    if (table->IsResetAllRequest)
        TableResetoptions(table);
    if (table->IsInitializing)
    {
        // Initialize
        table->optionsOffset = -1;
        table->IsSortSpecsDirty = true;
        table->IsoptionsDirty = true; // Records itself into .ini file even when in default state (#7934)
    ...
    // Load options
    if (table->IsoptionsRequestLoad)
        TableLoadoptions(table);
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        // Update Enabled state, mark options and sort specs dirty
...
void    ImGui::EndTable()
{
    ...
    // Save options
    if (table->IsoptionsDirty)
        TableSaveoptions(table);
...
// Called in TableSetupColumn() when initializing and in TableLoadoptions() for defaults before applying stored options.
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    if (table->IsInitializing)
    {
        ImGuiTableFlags init_flags = ~table->optionsLoadedFlags;
...
bool ImGui::TableBeginContextMenuPopup(ImGuiTable* table)
{
    if (!table->IsContextPopupOpen || table->InstanceCurrent != table->InstanceInteracted)
        return false;
    const ImGuiID context_menu_id = ImHashStr("##ContextMenu", 0, table->ID);
    if (BeginPopupEx(context_menu_id, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedoptions))
        return true;
...
//-------------------------------------------------------------------------
// - TableoptionsInit() [Internal]
// - TableoptionsCalcChunkSize() [Internal]
// - TableoptionsCreate() [Internal]
// - TableoptionsFindByID() [Internal]
// - TableGetBoundoptions() [Internal]
// - TableResetoptions()
// - TableSaveoptions() [Internal]
// - TableLoadoptions() [Internal]
// - TableoptionsHandler_ClearAll() [Internal]
// - TableoptionsHandler_ApplyAll() [Internal]
// - TableoptionsHandler_ReadOpen() [Internal]
// - TableoptionsHandler_ReadLine() [Internal]
// - TableoptionsHandler_WriteAll() [Internal]
// - TableoptionsInstallHandler() [Internal]
//-------------------------------------------------------------------------
// [Init] 1: TableoptionsHandler_ReadXXXX()   Load and parse .ini file into Tableoptions.
// [Main] 2: TableLoadoptions()               When table is created, bind Table to Tableoptions, serialize Tableoptions data into Table.
// [Main] 3: TableSaveoptions()               When table properties are modified, serialize Table data into bound or new Tableoptions, mark .ini as dirty.
// [Main] 4: TableoptionsHandler_WriteAll()   When .ini file is dirty (which can come from other source), save Tableoptions into .ini file.
//-------------------------------------------------------------------------

// Clear and initialize empty options instance
static void TableoptionsInit(ImGuiTableoptions* options, ImGuiID id, int columns_count, int columns_count_max)
{
    IM_PLACEMENT_NEW(options) ImGuiTableoptions();
    ImGuiTableColumnoptions* options_column = options->GetColumnoptions();
    for (int n = 0; n < columns_count_max; n++, options_column++)
        IM_PLACEMENT_NEW(options_column) ImGuiTableColumnoptions();
    options->ID = id;
    options->ColumnsCount = (ImGuiTableColumnIdx)columns_count;
    options->ColumnsCountMax = (ImGuiTableColumnIdx)columns_count_max;
    options->WantApply = true;
}

static size_t TableoptionsCalcChunkSize(int columns_count)
{
    return sizeof(ImGuiTableoptions) + (size_t)columns_count * sizeof(ImGuiTableColumnoptions);
}

ImGuiTableoptions* ImGui::TableoptionsCreate(ImGuiID id, int columns_count)
{
    ImGuiContext& g = *GImGui;
    ImGuiTableoptions* options = g.optionsTables.alloc_chunk(TableoptionsCalcChunkSize(columns_count));
    TableoptionsInit(options, id, columns_count, columns_count);
    return options;
}

// Find existing options
ImGuiTableoptions* ImGui::TableoptionsFindByID(ImGuiID id)
{
    // FIXME-OPT: Might want to store a lookup map for this?
    ImGuiContext& g = *GImGui;
    for (ImGuiTableoptions* options = g.optionsTables.begin(); options != NULL; options = g.optionsTables.next_chunk(options))
        if (options->ID == id)
            return options;
    return NULL;
}

// Get options for a given table, NULL if none
ImGuiTableoptions* ImGui::TableGetBoundoptions(ImGuiTable* table)
{
    if (table->optionsOffset != -1)
    {
        ImGuiContext& g = *GImGui;
        ImGuiTableoptions* options = g.optionsTables.ptr_from_offset(table->optionsOffset);
        IM_ASSERT(options->ID == table->ID);
        if (options->ColumnsCountMax >= table->ColumnsCount)
            return options; // OK
        options->ID = 0; // Invalidate storage, we won't fit because of a count change
    }
    return NULL;
}

// Restore initial state of table (with or without saved options)
void ImGui::TableResetoptions(ImGuiTable* table)
{
    table->IsInitializing = table->IsoptionsDirty = true;
    table->IsResetAllRequest = false;
    table->IsoptionsRequestLoad = false;                   // Don't reload from ini
    table->optionsLoadedFlags = ImGuiTableFlags_None;      // Mark as nothing loaded so our initialized data becomes authoritative
}

void ImGui::TableSaveoptions(ImGuiTable* table)
{
    table->IsoptionsDirty = false;
    if (table->Flags & ImGuiTableFlags_NoSavedoptions)
        return;

    // Bind or create options data
    ImGuiContext& g = *GImGui;
    ImGuiTableoptions* options = TableGetBoundoptions(table);
    if (options == NULL)
    {
        options = TableoptionsCreate(table->ID, table->ColumnsCount);
        table->optionsOffset = g.optionsTables.offset_from_ptr(options);
    }
    options->ColumnsCount = (ImGuiTableColumnIdx)table->ColumnsCount;

    // Serialize ImGuiTable/ImGuiTableColumn into ImGuiTableoptions/ImGuiTableColumnoptions
    IM_ASSERT(options->ID == table->ID);
    IM_ASSERT(options->ColumnsCount == table->ColumnsCount && options->ColumnsCountMax >= options->ColumnsCount);
    ImGuiTableColumn* column = table->Columns.Data;
    ImGuiTableColumnoptions* column_options = options->GetColumnoptions();

    bool save_ref_scale = false;
    options->SaveFlags = ImGuiTableFlags_None;
    for (int n = 0; n < table->ColumnsCount; n++, column++, column_options++)
    {
        const float width_or_weight = (column->Flags & ImGuiTableColumnFlags_WidthStretch) ? column->StretchWeight : column->WidthRequest;
        column_options->WidthOrWeight = width_or_weight;
        column_options->Index = (ImGuiTableColumnIdx)n;
        column_options->DisplayOrder = column->DisplayOrder;
        column_options->SortOrder = column->SortOrder;
        column_options->SortDirection = column->SortDirection;
        column_options->IsEnabled = column->IsUserEnabled;
        column_options->IsStretch = (column->Flags & ImGuiTableColumnFlags_WidthStretch) ? 1 : 0;
        ...
        if (width_or_weight != column->InitStretchWeightOrWidth)
            options->SaveFlags |= ImGuiTableFlags_Resizable;
        if (column->DisplayOrder != n)
            options->SaveFlags |= ImGuiTableFlags_Reorderable;
        if (column->SortOrder != -1)
            options->SaveFlags |= ImGuiTableFlags_Sortable;
        if (column->IsUserEnabled != ((column->Flags & ImGuiTableColumnFlags_DefaultHide) == 0))
            options->SaveFlags |= ImGuiTableFlags_Hideable;
    }
    options->SaveFlags &= table->Flags;
    options->RefScale = save_ref_scale ? table->RefScale : 0.0f;

    MarkInioptionsDirty();
}

void ImGui::TableLoadoptions(ImGuiTable* table)
{
    ImGuiContext& g = *GImGui;
    table->IsoptionsRequestLoad = false;
    if (table->Flags & ImGuiTableFlags_NoSavedoptions)
        return;

    // Bind options
    ImGuiTableoptions* options;
    if (table->optionsOffset == -1)
    {
        options = TableoptionsFindByID(table->ID);
        if (options == NULL)
            return;
        if (options->ColumnsCount != table->ColumnsCount) // Allow options if columns count changed. We could otherwise decide to return...
            table->IsoptionsDirty = true;
        table->optionsOffset = g.optionsTables.offset_from_ptr(options);
    }
    else
    {
        options = TableGetBoundoptions(table);
    }

    table->optionsLoadedFlags = options->SaveFlags;
    table->RefScale = options->RefScale;

    // Initialize default columns options
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ImGuiTableColumn* column = &table->Columns[column_n];
    ...
    // Serialize ImGuiTableoptions/ImGuiTableColumnoptions into ImGuiTable/ImGuiTableColumn
    ImGuiTableColumnoptions* column_options = options->GetColumnoptions();
    ImU64 display_order_mask = 0;
    for (int data_n = 0; data_n < options->ColumnsCount; data_n++, column_options++)
    {
        int column_n = column_options->Index;
        if (column_n < 0 || column_n >= table->ColumnsCount)
            continue;

        ImGuiTableColumn* column = &table->Columns[column_n];
        if (options->SaveFlags & ImGuiTableFlags_Resizable)
        {
            if (column_options->IsStretch)
                column->StretchWeight = column_options->WidthOrWeight;
            else
                column->WidthRequest = column_options->WidthOrWeight;
        }
        if (options->SaveFlags & ImGuiTableFlags_Reorderable)
            column->DisplayOrder = column_options->DisplayOrder;
        display_order_mask |= (ImU64)1 << column->DisplayOrder;
        if ((options->SaveFlags & ImGuiTableFlags_Hideable) && column_options->IsEnabled != -1)
            column->IsUserEnabled = column->IsUserEnabledNextFrame = column_options->IsEnabled == 1;
        column->SortOrder = column_options->SortOrder;
        column->SortDirection = column_options->SortDirection;
    }

    // Validate and fix invalid display order data
    const ImU64 expected_display_order_mask = (options->ColumnsCount == 64) ? ~0 : ((ImU64)1 << options->ColumnsCount) - 1;
...
static void TableoptionsHandler_ClearAll(ImGuiContext* ctx, ImGuioptionsHandler*)
{
    ImGuiContext& g = *ctx;
    for (int i = 0; i != g.Tables.GetMapSize(); i++)
        if (ImGuiTable* table = g.Tables.TryGetMapData(i))
            table->optionsOffset = -1;
    g.optionsTables.clear();
}

// Apply to existing windows (if any)
static void TableoptionsHandler_ApplyAll(ImGuiContext* ctx, ImGuioptionsHandler*)
{
    ImGuiContext& g = *ctx;
    for (int i = 0; i != g.Tables.GetMapSize(); i++)
        if (ImGuiTable* table = g.Tables.TryGetMapData(i))
        {
            table->IsoptionsRequestLoad = true;
            table->optionsOffset = -1;
        }
}

static void* TableoptionsHandler_ReadOpen(ImGuiContext*, ImGuioptionsHandler*, const char* name)
{
    ImGuiID id = 0;
    int columns_count = 0;
    ...
    if (ImGuiTableoptions* options = ImGui::TableoptionsFindByID(id))
    {
        if (options->ColumnsCountMax >= columns_count)
        {
            TableoptionsInit(options, id, columns_count, options->ColumnsCountMax); // Recycle
            return options;
        }
        options->ID = 0; // Invalidate storage, we won't fit because of a count change
    }
    return ImGui::TableoptionsCreate(id, columns_count);
}

static void TableoptionsHandler_ReadLine(ImGuiContext*, ImGuioptionsHandler*, void* entry, const char* line)
{
    // "Column 0  UserID=0x42AD2D21 Width=100 Visible=1 Order=0 Sort=0v"
    ImGuiTableoptions* options = (ImGuiTableoptions*)entry;
    float f = 0.0f;
    int column_n = 0, r = 0, n = 0;

    if (sscanf(line, "RefScale=%f", &f) == 1) { options->RefScale = f; return; }

    if (sscanf(line, "Column %d%n", &column_n, &r) == 1)
    {
        if (column_n < 0 || column_n >= options->ColumnsCount)
            return;
        line = ImStrSkipBlank(line + r);
        char c = 0;
        ImGuiTableColumnoptions* column = options->GetColumnoptions() + column_n;
        column->Index = (ImGuiTableColumnIdx)column_n;
        if (sscanf(line, "UserID=0x%08X%n", (ImU32*)&n, &r)==1) { line = ImStrSkipBlank(line + r); column->UserID = (ImGuiID)n; }
        if (sscanf(line, "Width=%d%n", &n, &r) == 1)            { line = ImStrSkipBlank(line + r); column->WidthOrWeight = (float)n; column->IsStretch = 0; options->SaveFlags |= ImGuiTableFlags_Resizable; }
        if (sscanf(line, "Weight=%f%n", &f, &r) == 1)           { line = ImStrSkipBlank(line + r); column->WidthOrWeight = f; column->IsStretch = 1; options->SaveFlags |= ImGuiTableFlags_Resizable; }
        if (sscanf(line, "Visible=%d%n", &n, &r) == 1)          { line = ImStrSkipBlank(line + r); column->IsEnabled = (ImU8)n; options->SaveFlags |= ImGuiTableFlags_Hideable; }
        if (sscanf(line, "Order=%d%n", &n, &r) == 1)            { line = ImStrSkipBlank(line + r); column->DisplayOrder = (ImGuiTableColumnIdx)n; options->SaveFlags |= ImGuiTableFlags_Reorderable; }
        if (sscanf(line, "Sort=%d%c%n", &n, &c, &r) == 2)       { line = ImStrSkipBlank(line + r); column->SortOrder = (ImGuiTableColumnIdx)n; column->SortDirection = (c == '^') ? ImGuiSortDirection_Descending : ImGuiSortDirection_Ascending; options->SaveFlags |= ImGuiTableFlags_Sortable; }
    }
}

static void TableoptionsHandler_WriteAll(ImGuiContext* ctx, ImGuioptionsHandler* handler, ImGuiTextBuffer* buf)
{
    ImGuiContext& g = *ctx;
    for (ImGuiTableoptions* options = g.optionsTables.begin(); options != NULL; options = g.optionsTables.next_chunk(options))
    {
        if (options->ID == 0) // Skip ditched options
            continue;

        // TableSaveoptions() may clear some of those flags when we establish that the data can be stripped
        // (e.g. Order was unchanged)
        const bool save_size    = (options->SaveFlags & ImGuiTableFlags_Resizable) != 0;
        const bool save_visible = (options->SaveFlags & ImGuiTableFlags_Hideable) != 0;
        const bool save_order   = (options->SaveFlags & ImGuiTableFlags_Reorderable) != 0;
        const bool save_sort    = (options->SaveFlags & ImGuiTableFlags_Sortable) != 0;
        // We need to save the [Table] entry even if all the bools are false, since this records a table with "default options".

        buf->reserve(buf->size() + 30 + options->ColumnsCount * 50); // ballpark reserve
        buf->appendf("[%s][0x%08X,%d]\n", handler->TypeName, options->ID, options->ColumnsCount);
        if (options->RefScale != 0.0f)
            buf->appendf("RefScale=%g\n", options->RefScale);
        ImGuiTableColumnoptions* column = options->GetColumnoptions();
        for (int column_n = 0; column_n < options->ColumnsCount; column_n++, column++)
        {
            // "Column 0  UserID=0x42AD2D21 Width=100 Visible=1 Order=0 Sort=0v"
...
void ImGui::TableoptionsAddoptionsHandler()
{
    ImGuioptionsHandler ini_handler;
    ini_handler.TypeName = "Table";
    ini_handler.TypeHash = ImHashStr("Table");
    ini_handler.ClearAllFn = TableoptionsHandler_ClearAll;
    ini_handler.ReadOpenFn = TableoptionsHandler_ReadOpen;
    ini_handler.ReadLineFn = TableoptionsHandler_ReadLine;
    ini_handler.ApplyAllFn = TableoptionsHandler_ApplyAll;
    ini_handler.WriteAllFn = TableoptionsHandler_WriteAll;
    AddoptionsHandler(&ini_handler);
}
...
// - TableGcCompactoptions() [Internal]
//-------------------------------------------------------------------------
...
// Compact and remove unused options data (currently only used by TestEngine)
void ImGui::TableGcCompactoptions()
{
    ImGuiContext& g = *GImGui;
    int required_memory = 0;
    for (ImGuiTableoptions* options = g.optionsTables.begin(); options != NULL; options = g.optionsTables.next_chunk(options))
        if (options->ID != 0)
            required_memory += (int)TableoptionsCalcChunkSize(options->ColumnsCount);
    if (required_memory == g.optionsTables.Buf.Size)
        return;
    ImChunkStream<ImGuiTableoptions> new_chunk_stream;
    new_chunk_stream.Buf.reserve(required_memory);
    for (ImGuiTableoptions* options = g.optionsTables.begin(); options != NULL; options = g.optionsTables.next_chunk(options))
        if (options->ID != 0)
            memcpy(new_chunk_stream.alloc_chunk(TableoptionsCalcChunkSize(options->ColumnsCount)), options, TableoptionsCalcChunkSize(options->ColumnsCount));
    g.optionsTables.swap(new_chunk_stream);
}
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    bool clear_options = SmallButton("Clear options");
    ...
    if (ImGuiTableoptions* options = TableGetBoundoptions(table))
        DebugNodeTableoptions(options);
    if (clear_options)
        table->IsResetAllRequest = true;
    TreePop();
}

void ImGui::DebugNodeTableoptions(ImGuiTableoptions* options)
{
    if (!TreeNode((void*)(intptr_t)options->ID, "options 0x%08X (%d columns)", options->ID, options->ColumnsCount))
        return;
    BulletText("SaveFlags: 0x%08X", options->SaveFlags);
    BulletText("ColumnsCount: %d (max %d)", options->ColumnsCount, options->ColumnsCountMax);
    for (int n = 0; n < options->ColumnsCount; n++)
    {
        ImGuiTableColumnoptions* column_options = &options->GetColumnoptions()[n];
        ImGuiSortDirection sort_dir = (column_options->SortOrder != -1) ? (ImGuiSortDirection)column_options->SortDirection : ImGuiSortDirection_None;
        BulletText("Column %d Order %d SortOrder %d %s Vis %d %s %7.3f UserID 0x%08X",
            n, column_options->DisplayOrder, column_options->SortOrder,
            (sort_dir == ImGuiSortDirection_Ascending) ? "Asc" : (sort_dir == ImGuiSortDirection_Descending) ? "Des" : "---",
            column_options->IsEnabled, column_options->IsStretch ? "Weight" : "Width ", column_options->WidthOrWeight, column_options->UserID);
    }
    TreePop();
}
...
void ImGui::DebugNodeTableoptions(ImGuiTableoptions*) {}
