...
struct ImGuioptionsHandler;        // Storage for one type registered in the .ini file
...
struct ImGuiTableoptions;          // Storage for a table .ini options
struct ImGuiTableColumnsoptions;   // Storage for a column .ini options
...
struct ImGuiWindowoptions;         // Storage for a window .ini options (we keep one of those even if the actual window wasn't instanced during this session)
...
// Enforce cdecl calling convention for functions called by the standard library, in case compilation options changed the default to e.g. __vectorcall
#ifdef _MSC_VER
...
// This is used by options to store persistent data while reducing allocation count.
...
// Because we never destroy or rename ImGuiWindowoptions, we can store the names in a separate buffer easily.
...
struct ImGuiWindowoptions
{
    ...
    bool        WantDelete;     // Set to invalidate/delete the options entry

    ImGuiWindowoptions()       { memset(this, 0, sizeof(*this)); }
...
struct ImGuioptionsHandler
{
    ...
    void        (*ClearAllFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler);                                // Clear all options data
    void        (*ReadInitFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler);                                // Read: Called before reading (in registration order)
    void*       (*ReadOpenFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler, const char* name);              // Read: Called when entering into a new ini entry e.g. "[Window][Name]"
    void        (*ReadLineFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler, void* entry, const char* line); // Read: Called for every line of text within an ini entry
    void        (*ApplyAllFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler);                                // Read: Called after reading (in registration order)
    void        (*WriteAllFn)(ImGuiContext* ctx, ImGuioptionsHandler* handler, ImGuiTextBuffer* out_buf);      // Write: Output every entries into 'out_buf'
    void*       UserData;
...
struct ImGuiContext
{
    ...
    float                   DragCurrentAccum;                   // Accumulator for dragging modification. Always high-precision, not rounded by end-user precision options
    ...
    // options
    bool                    optionsLoaded;
    float                   optionsDirtyTimer;                 // Save .ini options to memory when time reaches zero
    ImGuiTextBuffer         optionsIniData;                    // In memory .ini options
    ImVector<ImGuioptionsHandler>      optionsHandlers;       // List of .ini options handlers
    ImChunkStream<ImGuiWindowoptions>  optionsWindows;        // ImGuiWindow .ini options entries
    ImChunkStream<ImGuiTableoptions>   optionsTables;         // ImGuiTable .ini options entries
...
struct IMGUI_API ImGuiWindow
{
    ...
    int                     optionsOffset;                     // Offset into optionsWindows[] (offsets are always valid as we only grow the array from the back)
...
enum ImGuiTabBarFlagsPrivate_
{
    ...
    ImGuiTabBarFlags_Saveoptions               = 1 << 22,  // FIXME: options are handled by the docking system, this only request the tab bar to mark options dirty when reordering tabs
};
...
struct IMGUI_API ImGuiTable
{
    ...
    ImGuiTableFlags             optionsLoadedFlags;        // Which data were loaded from the .ini file (e.g. when order is not altered we won't save order)
    int                         optionsOffset;             // Offset in g.optionsTables
    int                         LastFrameActive;
    ...
    bool                        IsoptionsRequestLoad;
    bool                        IsoptionsDirty;            // Set when table options have changed and needs to be reported into ImGuiTableSetttings data.
...
// sizeof() ~ 16
struct ImGuiTableColumnoptions
{
    float                   WidthOrWeight;
    ImGuiID                 UserID;
...
// This is designed to be stored in a single ImChunkStream (1 header followed by N ImGuiTableColumnoptions, etc.)
struct ImGuiTableoptions
{
    ImGuiID                     ID;                     // Set to 0 to invalidate/delete the setting
    ImGuiTableFlags             SaveFlags;              // Indicate data we want to save using the Resizable/Reorderable/Sortable/Hideable flags (could be using its own flags..)
    float                       RefScale;               // Reference scale to be able to rescale columns on font/dpi changes.
    ImGuiTableColumnIdx         ColumnsCount;
    ImGuiTableColumnIdx         ColumnsCountMax;        // Maximum number of columns this options instance can store, we can recycle a options instance with lower number of columns but not higher
    bool                        WantApply;              // Set when loaded from .ini data (to enable merging/loading .ini data into an already running context)

    ImGuiTableoptions()        { memset(this, 0, sizeof(*this)); }
    ImGuiTableColumnoptions*   GetColumnoptions()     { return (ImGuiTableColumnoptions*)(this + 1); }
};
...
namespace ImGui
{
    ...
    IMGUI_API void                  MarkInioptionsDirty(ImGuiWindow* window);
    IMGUI_API void                  ClearInioptions();
    IMGUI_API void                  AddoptionsHandler(const ImGuioptionsHandler* handler);
    IMGUI_API void                  RemoveoptionsHandler(const char* type_name);
    IMGUI_API ImGuioptionsHandler* FindoptionsHandler(const char* type_name);

    // options - Windows
    IMGUI_API ImGuiWindowoptions*  CreateNewWindowoptions(const char* name);
    IMGUI_API ImGuiWindowoptions*  FindWindowoptionsByID(ImGuiID id);
    IMGUI_API ImGuiWindowoptions*  FindWindowoptionsByWindow(ImGuiWindow* window);
    IMGUI_API void                  ClearWindowoptions(const char* name);
...
//#endif
    ...
    IMGUI_API void          TableGcCompactoptions();

    // Tables: options
    IMGUI_API void                  TableLoadoptions(ImGuiTable* table);
    IMGUI_API void                  TableSaveoptions(ImGuiTable* table);
    IMGUI_API void                  TableResetoptions(ImGuiTable* table);
    IMGUI_API ImGuiTableoptions*   TableGetBoundoptions(ImGuiTable* table);
    IMGUI_API void                  TableoptionsAddoptionsHandler();
    IMGUI_API ImGuiTableoptions*   TableoptionsCreate(ImGuiID id, int columns_count);
    IMGUI_API ImGuiTableoptions*   TableoptionsFindByID(ImGuiID id);
    ...
    IMGUI_API void          DebugNodeTableoptions(ImGuiTableoptions* options);
    ...
    IMGUI_API void          DebugNodeWindowoptions(ImGuiWindowoptions* options);
