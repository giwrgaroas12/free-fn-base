...
namespace ImGui
{
    ...
    IMGUI_API float         CalcItemWidth();                                                // width of item given pushed options and current cursor position. NOT necessarily the width of last item unlike most 'Item' functions.
    ...
    IMGUI_API bool          IsMouseHoveringRect(const ImVec2& r_min, const ImVec2& r_max, bool clip = true);// is mouse hovering given bounding rect (in screen space). clipped by current clipping options, but disregarding of other consideration of focus/window ordering/popup-block.
    ...
    // options/.Ini Utilities
    // - The disk functions are automatically called if io.IniFilename != NULL (default is "imgui.ini").
    // - Set io.IniFilename to NULL to load/save manually. Read io.WantSaveInioptions description about handling .ini saving manually.
    // - Important: default value "imgui.ini" is relative to current working dir! Most apps will want to lock this to an absolute path (e.g. same path as executables).
    IMGUI_API void          LoadInioptionsFromDisk(const char* ini_filename);                  // call after CreateContext() and before the first call to NewFrame(). NewFrame() automatically calls LoadInioptionsFromDisk(io.IniFilename).
    IMGUI_API void          LoadInioptionsFromMemory(const char* ini_data, size_t ini_size=0); // call after CreateContext() and before the first call to NewFrame() to provide .ini data from your own data source.
    IMGUI_API void          SaveInioptionsToDisk(const char* ini_filename);                    // this is automatically called (if io.IniFilename is not empty) a few seconds after any modification that should be reflected in the .ini file (and also by DestroyContext).
    IMGUI_API const char*   SaveInioptionsToMemory(size_t* out_ini_size = NULL);               // return a zero-terminated string with the .ini data which you can save by your own mean. call when io.WantSaveInioptions is set, then save data by your own mean and clear io.WantSaveInioptions.
...
enum ImGuiChildFlags_
{
    ...
    ImGuiChildFlags_ResizeX                 = 1 << 2,   // Allow resize from right border (layout direction). Enable .ini saving (unless ImGuiWindowFlags_NoSavedoptions passed to window flags)
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoSavedoptions            = 1 << 4,   // Disable persisting columns order, width and sort options in the .ini file.
...
// [SECTION] ImGuiIO
//-----------------------------------------------------------------------------
// Communicate most options and inputs/outputs to Dear ImGui using this structure.
...
struct ImGuiIO
{
    ...
    const char* IniFilename;                    // = "imgui.ini"    // Path to .ini file (important: default "imgui.ini" is relative to current working dir!). Set NULL to disable automatic .ini loading/saving or if you want to manually call LoadInioptionsXXX() / SaveInioptionsXXX() functions.
    ...
    // - Recovery after error/exception: record stack sizes with ErrorRecoveryStoreState(), disable assert, set log callback (to e.g. trigger high-level breakpoint), recover with ErrorRecoveryTryToRecoverState(), restore options.
    ...
    bool        ConfigDebugInioptions;         // = false          // Save .ini data with extra comments (particularly helpful for Docking, but makes saving slower)
    ...
    bool        WantSaveInioptions;                // When manual .ini load/save is active (io.IniFilename == NULL), this will be set to notify your application that you can call SaveInioptionsToMemory() and save yourself. Important: clear io.WantSaveInioptions yourself after saving!
...
// Render state is not reset by default because they are many perfectly useful way of altering render state (e.g. changing shader/blending options before an Image call).
...
// Flags for ImDrawList instance. Those are set automatically by ImGui:: functions from ImGuiIO options, and generally not manipulated directly.
...
struct ImFontConfig
{
    ...
    unsigned int    FontBuilderFlags;       // 0        // options for custom font builder. THIS IS BUILDER IMPLEMENTATION DEPENDENT. Leave as zero if unsure.
...
struct ImFontAtlas
{
    ...
    // - Note: this API may be redesigned later in order to support multi-monitor varying DPI options.
